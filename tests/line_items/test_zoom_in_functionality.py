...
def test_TC_15_zoom_in_functionality(page):
    ...
    after_zoom_text = zoom_label.inner_text()
    ...
    assert after_zoom > before_zoom, \
        (
            f"Zoom did not increase. "
            f"Before={before_zoom}% "
            f"After={after_zoom}%"
        )

    print(
        "PASS : PDF zoom level increased successfully"
    )